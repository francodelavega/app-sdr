...
export default function SummaryCards({ appointments, preDemoTotal, loading }) {
  const stats = useMemo(() => {
    const now     = new Date()
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      {cards.map(card => (
        <div key={card.label} className={`relative rounded-xl p-4 bg-white dark:bg-navy-700 border shadow-sm ${
